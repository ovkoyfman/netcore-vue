<template>
  <div class="container">
    <div class="form-group">
        <flat-pickr v-model="flatDates" class="form-control" placeholder="Select Date..." :config="config" @on-close="closed">
          </flat-pickr>
    </div>
      <div v-if="formData.dates.length === 0" class="well">
        <p>Please select date range to build room block</p>
      </div>
  <div v-else id="form-div" @click.prevent.stop="viewMenu = false">
    {{formData.dates}}
    ...
    <draggable :element="'form'" @submit.prevent class="border" :options="{handle:'.handleCategoryParent'}" :class="dropClass">

      <template v-for="(item, parentIndex) in formData.components" class="room-block">
        <table>
          <draggable :element="'tbody'" :options="{handle:'.handleParent', group:'tables'}" :listId="parentIndex" :id="parentIndex"
            :class="formData.components[parentIndex].disabled ? '' : 'edit'">
            <tr class="category">
              ...
              <td colspan="2">
                <b-input :disabled="formData.components[parentIndex].disabled" />
              </td>
              <td :colspan="formData.template.children[0].room.length*2-1">
                <b-button size="sm" class="remove-category" @click="removeCategory($event)">Remove Category</b-button>
              </td>
            </tr>

            <template v-for="(item, index) in formData.components[parentIndex].children">
  ...
  export default {
    ...
    methods: {
      ...
      closed: function (selectedDates, dateStr, instance) {
        ...
        var roomRows = this.formData.components;

        for(let i = 0; i < roomRows.length; i++){
          //console.log(roomRows[i].children);
          for(let j = 0; j < roomRows[i].children.length; j++){
            //console.log(roomRows[i].children[j].room);
            var temparr = [{selected: "Select One"}]
            for(let k = 0; k < dateRange.length; k++){
              temparr.push({Rate: "0", Qty: "0"})
            }
            roomRows[i].children[j].room = temparr;
            temparr = [];
          }
        }

        //console.log(dateRange);
        this.formData.dates = dateRange;
